...
```sh
# 构建镜像
docker build -t "qq1060656096/php:5.4.45-kafka" 5.4.45-kafka
# 启动容器
docker run -tid --name php-kafka qq1060656096/php:5.4.45-kafka
# 进入容器
...
docker build -t "qq1060656096/php:5.4.45-kafka" 5.4.45-kafka
# 启动容器
docker run -tid -p 1090:9000 --name php-kafka qq1060656096/php:5.4.45-kafka /usr/local/php/sbin/php-fpm -F
# 进入容器
docker exec -ti php-kafka /bin/bash
# 查看容器nginx状态
